...
def main(image_dir, output_dir):
    ...
    # Get higher red hue
    higher_red_hue = create_hue_mask(hsv_image, [156, 100, 100], [179, 255, 255])
    if output_dir:
    ...
    circles = cv2.HoughCircles(image_gray, cv2.cv.CV_HOUGH_GRADIENT, 2.5, 300, param1 = 150, param2 = 50)
    ...
    if output_dir:
        ...
        result_image_path = os.path.join(output_dir, str(image_dir.split('/')[-1]))
...
if __name__ == '__main__':
    image_dir = '/home/whj/gitrepo/python_learn/opencv_light_detection-master/images/'
    output_dir = os.path.join(image_dir, 'debug/')
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)
    files = os.listdir(image_dir)
    # for image in files:
    #    print image + ':' +  main(image_dir+image, output_dir)
    image = image_dir + '111.jpg'
    # image = image_dir + 'on_102954_557.jpg'
    print image + ':' +  main(image, output_dir)
...
#             dir_list.pop()
#         dir_list.pop()
#         output_dir = '/'.join(dir_list)+'/debug/'
#
#     return main(image_dir, output_dir)
#
# run(host='192.168.10.155', port=2334)
